...
%%%%  Pipeline Goubault (2023) — REPÈRE SEGMENT (local)                %%%%
%%%%                                                                   %%%%
...
%% -----------------------------------------------------------------------
%  AUTO-TEST QUATERNION — détermine la convention (scalaire 1er ou dernier)
%  En T-pose, le pelvis/L5 est quasi aligné sur le repère global : son
%  quaternion doit être proche de l'identité, donc |scalaire| ~ 1 et les
%  trois composantes vectorielles ~ 0. La position du terme dominant
%  révèle la convention.
%  -----------------------------------------------------------------------
fprintf('=== Auto-test quaternion ===\n');
tmp_q = load(xsens_files{1});
XF_q  = tmp_q.XSens_Fatigue;
s_q   = fieldnames(XF_q);  c_q = fieldnames(XF_q.(s_q{1}));
q_tp  = double(XF_q.(s_q{1}).(c_q{1}).segmentData(Seg_idx(1)).tpose.orientation);
q_tp  = q_tp(:)';
fprintf('  T-pose L5 : [%+.4f %+.4f %+.4f %+.4f]\n', q_tp);
[~, i_dom] = max(abs(q_tp));
if i_dom == 1
    quat_scalar_last = false;
    fprintf('  Terme dominant en position 1 → convention SCALAIRE EN PREMIER\n');
elseif i_dom == 4
    quat_scalar_last = true;
    fprintf('  Terme dominant en position 4 → convention SCALAIRE EN DERNIER\n');
else
    error(['Terme dominant en position %d : la T-pose du segment de référence ' ...
           'n''est pas proche de l''identité. Vérifier manuellement la convention.'], i_dom);
end
r_dom = abs(q_tp(i_dom)) / max(abs(q_tp([1:i_dom-1, i_dom+1:4])));
fprintf('  Rapport dominant/second : %.1f\n', r_dom);
if r_dom < 3
    warning('Rapport faible (%.1f) : la détection de convention est peu fiable.', r_dom);
end

% Vérification numérique : la reprojection doit conserver la magnitude
a_chk = double(XF_q.(s_q{1}).(c_q{1}).segmentData(Seg_idx(1)).acceleration);
o_chk = double(XF_q.(s_q{1}).(c_q{1}).segmentData(Seg_idx(1)).orientation);
if quat_scalar_last, o_chk = o_chk(:,[4 1 2 3]); end
o_chk = o_chk ./ max(sqrt(sum(o_chk.^2,2)), 1e-12);
w_=o_chk(:,1); x_=o_chk(:,2); y_=o_chk(:,3); z_=o_chk(:,4);
a_loc_chk = [ (1-2*(y_.^2+z_.^2)).*a_chk(:,1) + (2*(x_.*y_+w_.*z_)).*a_chk(:,2) + (2*(x_.*z_-w_.*y_)).*a_chk(:,3), ...
              (2*(x_.*y_-w_.*z_)).*a_chk(:,1) + (1-2*(x_.^2+z_.^2)).*a_chk(:,2) + (2*(y_.*z_+w_.*x_)).*a_chk(:,3), ...
              (2*(x_.*z_+w_.*y_)).*a_chk(:,1) + (2*(y_.*z_-w_.*x_)).*a_chk(:,2) + (1-2*(x_.^2+y_.^2)).*a_chk(:,3) ];
err_mag = max(abs(vecnorm(a_loc_chk,2,2) - vecnorm(a_chk,2,2)));
fprintf('  Écart max de magnitude après reprojection : %.3e\n', err_mag);
if err_mag > 1e-9
    error('La reprojection ne conserve pas la magnitude — rotation invalide.');
else
    fprintf('  Magnitude conservée : rotation valide.\n');
end
% Part de variance déplacée entre axes (0 = repères identiques)
d_rel = mean(abs(a_loc_chk - a_chk), 'all') / mean(abs(a_chk), 'all');
fprintf('  Écart relatif moyen composantes global/local : %.1f%%\n', 100*d_rel);
fprintf('\n');
clear tmp_q XF_q s_q c_q q_tp a_chk o_chk w_ x_ y_ z_ a_loc_chk;
...
%  -----------------------------------------------------------------------
    ...
    function v_loc = rot_to_local(v, R11,R12,R13,R21,R22,R23,R31,R32,R33)
        % v_loc = R' * v  (R' = transposée => on lit R par colonnes)
        v_loc = [ R11.*v(:,1) + R21.*v(:,2) + R31.*v(:,3), ...
                  R12.*v(:,1) + R22.*v(:,2) + R32.*v(:,3), ...
                  R13.*v(:,1) + R23.*v(:,2) + R33.*v(:,3) ];
    end
        ...
        for iK = 1:numel(conditions)
            ...
            try
                ...
                for iSeg = 1:N_seg
                    ...
                    ori_raw{iSeg} = XF.(subj).(kbd).segmentData(Seg_idx(iSeg)).orientation;
                end
            catch ME
            ...
            %% --- REPROJECTION EN REPÈRE SEGMENT (local) ---
            %  a_local = R' * a_global, R = matrice de rotation segment->global
            %  La magnitude est invariante : toutes les features _Magnitude
            %  sont identiques à la version repère global. Seules X, Y, Z changent.
            for iSeg = 1:N_seg
                q = double(ori_raw{iSeg});
                if size(q,1) ~= N_frames || size(q,2) ~= 4
                    error('Quaternion de dimension inattendue (%dx%d) pour %s/%s seg %d', ...
                          size(q,1), size(q,2), subj, kbd, iSeg);
                end
                if quat_scalar_last
                    q = q(:, [4 1 2 3]);   % ramener en scalaire-en-premier
                end
                nq = sqrt(sum(q.^2, 2));  nq(nq < 1e-12) = 1;
                q  = q ./ nq;
                ...
                acc_raw{iSeg} = rot_to_local(double(acc_raw{iSeg}), ...
                                    R11,R12,R13,R21,R22,R23,R31,R32,R33);
                gyr_raw{iSeg} = rot_to_local(double(gyr_raw{iSeg}), ...
                                    R11,R12,R13,R21,R22,R23,R31,R32,R33);
            end
            ...
            rpe_frames = round(RPE_shape * N_frames);
...
%  SAUVEGARDE
%  -----------------------------------------------------------------------
save_path = fullfile(path_save,'SpectralFeatures_Ergo.mat');
save(save_path,'Results_Spectral','Seg_names','Seg_idx','sig_types',...
    ...
    'win_sec','half_bin_sec','RPE_shape','norm_level','quat_scalar_last',...
    'filt_order','filt_band','-v7.3');
fprintf('\nSauvegardé : SpectralFeatures_Ergo.mat\n');
