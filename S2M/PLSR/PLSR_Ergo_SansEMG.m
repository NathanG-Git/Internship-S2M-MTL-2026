%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%  PLSR + VIP ITERATIF — Dataset Robin — SANS EMG                    %%%%
%%%%  4 modèles : M1=SH/Std  M2=SH/Ergo  M3=LH/Std  M4=Pool/Std      %%%%
%%%%  1260 variables IMU uniquement (les 20 variables EMG sont exclues)  %%%%
...
F_spectral = load(fullfile(path_plsr, 'SpectralFeatures_Ergo.mat'));
fprintf('  OK\n\n');
...
%% -----------------------------------------------------------------------
%  LISTE DES 1260 VARIABLES
%  1260 features spectrales (SpectralFeatures_Ergo2) — EMG EXCLUES
%  -----------------------------------------------------------------------
...
% ── Features EMG — EXCLUES dans cette version ───────────────────────────
feat_emg = {};
src_emg  = {};
...
%  SAUVEGARDE
%  -----------------------------------------------------------------------
save(fullfile(path_save,'PLSR_Results_SansEMG.mat'), 'Results_PLSR');
fprintf('Sauvegardé : PLSR_Results_SansEMG.mat\n\n');
...
%  -----------------------------------------------------------------------
fprintf('Génération Fig_Tableau_Reduction_SansEMG...\n');
...
fig_tbl = figure('Name','Tableau_Reduction_SansEMG','NumberTitle','off',...
...
sgtitle('Tableau de réduction itérative — Sans EMG — Absolute Error (test CV) et %RPE (train) par seuil VIP',...
    'FontSize',11,'FontWeight','bold');

saveas(fig_tbl, fullfile(path_save,'Fig_Tableau_Reduction_SansEMG.png')); close(fig_tbl);
fprintf('Figure sauvegardée : Fig_Tableau_Reduction_SansEMG.png\n\n');
...
%  -----------------------------------------------------------------------
fig2 = figure('Name','VIP_IC95_SansEMG','NumberTitle','off',...
...
sgtitle('Stabilité VIP — Sans EMG — IC 95% sur 50 répétitions (top 10 par modèle)',...
    'FontSize',11,'FontWeight','bold');
saveas(fig2,fullfile(path_save,'Fig_VIP_IC95_SansEMG.png')); close(fig2);

fprintf('Figures sauvegardées :\n');
fprintf('  Fig_VIP_IC95_SansEMG.png\n');
...
fig_t1 = figure('Name','Tableau_Top10_PLSR_SansEMG','NumberTitle','off',...
    'Position',[30 30 1400 420],'Color','w');

sgtitle('Top 10 — PLSR Robin Sans EMG  (↑ B>0 | ↓ B<0  |  VIP Passe 2)',...
    'FontSize',11,'FontWeight','bold');
...
saveas(fig_t1,fullfile(path_save,'Tableau_Top10_PLSR_SansEMG.png')); close(fig_t1);
fprintf('Tableau sauvegardé : Tableau_Top10_PLSR_SansEMG.png\n');
...
fig_t2 = figure('Name','Tableau_Reference_SansEMG','NumberTitle','off',...
...
saveas(fig_t2,fullfile(path_save,'Tableau_Reference_SansEMG.png')); close(fig_t2);
fprintf('Tableau sauvegardé : Tableau_Reference_SansEMG.png\n');
