...
%%%% 5) Figures LMM %%%%
%%%% 6) Boucle de stabilité Group LASSO (100 graines) %%%%
%%%% 7) Sauvegarde %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% -----------------------------------------------------------------------
% ÉTAPE 2 : CONSTRUCTION DES TABLEAUX LONGS PAR BLOC
% -----------------------------------------------------------------------
fprintf('\n=== Étape 2 : Construction des tableaux longs par bloc ===\n');
...
X_G1_bloc = struct('SegMod',[],'SegAxis',[],'Goubault',[],'EMG',[]);
X_G2_bloc = struct('SegMod',[],'SegAxis',[],'Goubault',[],'EMG',[]);
...
for iv = 1:N_sig
    ...
    [x1,r1,s1] = buildBlock(Vi_1, M1, R1, N_bins);
    [x2,r2,s2] = buildBlock(Vi_2, M2, R2, N_bins);

    vnames_bloc.(src){end+1} = vn;
    X_G1_bloc.(src) = [X_G1_bloc.(src) x1];
    X_G2_bloc.(src) = [X_G2_bloc.(src) x2];
...
for ib = 1:4
    ...
    fprintf('  Bloc %-10s : %3d variables parentes | G1 n=%4d obs | G2 n=%4d obs\n', ...
        bn, length(vnames_bloc.(bn)), size(X_G1_bloc.(bn),1), size(X_G2_bloc.(bn),1));
end
...
for ib = 1:4
    ...
    for iGrp = 1:2
        if iGrp == 1
            X = X_G1_bloc.(bn); Y = RPE_G1_bloc.(bn); S = subj_G1_bloc.(bn); gname = 'G1';
        else
            X = X_G2_bloc.(bn); Y = RPE_G2_bloc.(bn); S = subj_G2_bloc.(bn); gname = 'G2';
        end

        ok_all = ~any(isnan(X)|isinf(X),2) & ~isnan(Y) & ~isinf(Y);
        X = X(ok_all,:); Yok = Y(ok_all); Sok = S(ok_all);
        N_obs = length(Yok);

        % Diagnostic si perte d'observations inattendue
        N_obs_attendu = length(unique(Sok)) * N_bins;
        if N_obs < N_obs_attendu * 0.9
            fprintf('    ATTENTION : %d obs après nettoyage (attendu ~%d) — NaN dans RPE ?\n',...
                N_obs, N_obs_attendu);
        end

        % K_FOLDS adaptatif selon le nombre de sujets disponibles
        % LOSO (Leave-One-Subject-Out) si N_subj <= 15 : gold standard biomécanique
        % K=3 si N_subj <= 20 : folds de ~7 sujets, SE stable
        % K=5 si N_subj > 20  : comportement standard
        uSubj_tmp = unique(Sok);
        N_subj_tmp = length(uSubj_tmp);
        if N_subj_tmp <= 15
            K_FOLDS_loc = N_subj_tmp;   % LOSO
            fprintf('\n  [%s | %s] : %d obs, %d vars, LOSO (%d folds)\n', bn, gname, N_obs, N_vars, K_FOLDS_loc);
        elseif N_subj_tmp <= 20
            K_FOLDS_loc = 3;
            fprintf('\n  [%s | %s] : %d obs, %d vars, K=%d folds\n', bn, gname, N_obs, N_vars, K_FOLDS_loc);
        else
            K_FOLDS_loc = K_FOLDS;      % K=5 par défaut
            fprintf('\n  [%s | %s] : %d obs, %d vars, K=%d folds\n', bn, gname, N_obs, N_vars, K_FOLDS_loc);
        end

        % --- Décomposition de Mundlak ---
        uSubj = unique(Sok);
        N_subj = length(uSubj);
        X_within = NaN(N_obs, N_vars);
        X_between = NaN(N_obs, N_vars);
        for iS = 1:N_subj
            idx_s = find(Sok == uSubj(iS));
            for iv = 1:N_vars
                mu_iv = mean(X(idx_s,iv), 'omitnan');
                X_between(idx_s,iv) = mu_iv;
                X_within(idx_s,iv) = X(idx_s,iv) - mu_iv;
            end
        end
...
for ib = 1:4
    ...
    saveas(fig_cv, fullfile(PathSave, sprintf('GroupLASSO_CVPath_%s.png',bn)));
    close(fig_cv);
    ...
    saveas(fig_beta, fullfile(PathSave, sprintf('GroupLASSO_Mundlak_Beta_%s.png',bn)));
    close(fig_beta);
end
...
for ib = 1:4
    ...
    for iGrp = 1:2
        ...
        col_names_lmm = cellfun(@(x) regexprep(x,'[^a-zA-Z0-9]','_'), ...
            vnames(sel_idx), 'UniformOutput', false);
        % Pas de lag-1 : la décomposition Mundlak (within/between) couplée
        % à l'effet aléatoire (1|Participant) gère la structure temporelle
        % et inter-individuelle sans introduire d'endogénéité.
...
for ib = 1:4
    ...
    fig_name = sprintf('Indiv_LMM_Bloc_%s_G1vsG2', bn);
...
grid on; box on; hold off;
saveas(gcf, fullfile(PathSave,'Indiv_LMM_R2Adj_ParBloc.png')); close(gcf);

%% -----------------------------------------------------------------------
% ÉTAPE 6 : BOUCLE DE STABILITÉ GROUP LASSO (100 GRAINES)
% -----------------------------------------------------------------------
fprintf('\n=== Étape 6 : Analyse de stabilité sur 100 graines ===\n');
seeds_stab = 1:100;
...
for iSeed = 1:N_stab
    ...
    fprintf('  -> Exécution de la simulation : Run %d/%d (Graine active : %d)...\n', iSeed, N_stab, seed_val);
    ...
    for ib = 1:4
        ...
        for iGrp = 1:2
            if iGrp==1
                X=X_G1_bloc.(bn); Y=RPE_G1_bloc.(bn); S=subj_G1_bloc.(bn);
            else
                X=X_G2_bloc.(bn); Y=RPE_G2_bloc.(bn); S=subj_G2_bloc.(bn);
            end

            ok = ~any(isnan(X)|isinf(X),2) & ~isnan(Y) & ~isinf(Y);
            if sum(ok) < 20, continue; end
            Xok=X(ok,:); Yok=Y(ok); Sok=S(ok); N_obs=length(Yok);
            ...
            for iS = 1:N_subj
                idx_s = find(Sok == uSubj(iS));
                for iv = 1:N_vars
                    mu_iv = mean(Xok(idx_s,iv),'omitnan');
                    X_between(idx_s,iv) = mu_iv;
                    X_within(idx_s,iv) = Xok(idx_s,iv) - mu_iv;
                end
            end
            ...
            try
                ...
                % Variable cible : RPE brut, sans lag-1
                % Mundlak + (1|Participant) gèrent la structure temporelle et inter-individuelle
                vnames_clean_s = cellfun(@(x) regexprep(x,'[^a-zA-Z0-9]','_'),vnames,'UniformOutput',false);
                Xsel_s = Xok(:,idx_sel_s);
                ...
                col_run=vnames_clean_s(idx_sel_s);
                ...
                for jj=1:length(idx_sel_s)
                    ...
                    % Trouver l'indice de cette variable dans col_run
                    idx_col_run = find(strcmp(col_run, vnames_clean_s{iv}));
...
for ib = 1:4
    ...
    for iGrp = 1:2
        ...
        saveas(fig_fp, fullfile(PathSave, sprintf('ForestPlot_%s_%s.png',bn,gname)));
        close(fig_fp);
...
for ib = 1:4
    ...
    fig_name=sprintf('Indiv_GroupLASSO_Stabilite_%s.png',bn);
...
% Permet de régénérer les figures sans relancer les 100 runs
...
save(fullfile(PathSave,'Stability_Archive.mat'), 'stability_archive', '-v7.3');
fprintf('Stabilité archivée : Stability_Archive.mat\n');

% --- Sauvegarde principale ---
save(fullfile(PathSave,'Indiv_LASSO_LMM_Results.mat'),...
    'sel_vnames','sel_source','mundlak_G1','mundlak_G2','lmm_G1','lmm_G2',...
    'stab_G1','stab_G2','sd_Y_global');
fprintf('\n========================================================================\n');
fprintf('  PIPELINE EXÉCUTÉ AVEC SUCCÈS | RÉSULTATS SAUVEGARDÉS ET EXPORTÉS\n');
fprintf('========================================================================\n');
...
% Variable cible : RPE brut (tous les intervalles t=1..N_bins)
% Variable prédictive : X(t) au même intervalle
% La structure temporelle est gérée par :
%   - la décomposition Mundlak (within = déviation par rapport à la moyenne du sujet)
%   - l'effet aléatoire (1|Participant) dans le LMM
% Note : subj_long stocke iG (indice local 1..N_subj), pas iP (indice absolu)
% pour garantir la cohérence entre blocs avec des Vi différents.
    N_subj  = length(Vi);
    N_obs   = N_subj * N_bins;
    X_long    = NaN(N_obs, 1);
    ...
    for iG = 1:N_subj
        ...
        for k = 1:N_bins
            row_idx = row_idx + 1;
            X_long(row_idx)    = x_subj(k);
            RPE_long(row_idx)  = rpe_subj(k);
