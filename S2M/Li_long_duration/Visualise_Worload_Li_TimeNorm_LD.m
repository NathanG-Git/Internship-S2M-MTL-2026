%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%   Visualization — Workload & RPE vs Temps normalisé (%) — G2 Long Duration   %%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %
    ...
    %
    % G2 = participants ayant joué les 12 minutes complètes.
    %   Pas de coupure RPE — toute la session est utilisée.
    %
    ...
    PathSave   = 'C:\Users\Nathan\OneDrive - Aix-Marseille Université\Documents\MATLAB\Piano_fatigue\Li_long_duration\';
    ...
    G2_Liszt = [3,8,10,11,12,13,14,16,17,18,19,24,25,26,27,33,34,36,40,41,43,47,48,50];
    ...
    %  -----------------------------------------------------------------------
    Mat_Total_Accel = NaN(length(G2_Liszt), N_bins);
    Mat_Total_Jerk  = NaN(length(G2_Liszt), N_bins);
    Mat_Axis_Accel  = NaN(length(G2_Liszt), N_bins, 3);
    Mat_Axis_Jerk   = NaN(length(G2_Liszt), N_bins, 3);
    Mat_Seg_Accel   = NaN(length(G2_Liszt), N_bins, N_seg);
    Mat_Seg_Jerk    = NaN(length(G2_Liszt), N_bins, N_seg);
    Mat_SegAxis_Accel = NaN(length(G2_Liszt), N_bins, N_seg, 3);  % 7 seg × 3 axes
    Mat_SegAxis_Jerk  = NaN(length(G2_Liszt), N_bins, N_seg, 3);
    Mat_RPE         = NaN(length(G2_Liszt), N_bins);
    
    cycles_info = load(fullfile(PathCycle, 'cycles_Li.mat'), 'participants');
    valid_subj  = false(length(G2_Liszt), 1);
    ...
    for iG = 1:length(G2_Liszt)
        ...
        if isempty(t_cycles_p)
            fprintf('  %s: pas de cycles Liszt dans cycles_Li, skip\n', SubjID);
            continue;
        end
    ...
    fprintf('\n%d / %d participants traités\n', sum(valid_subj), length(G2_Liszt));
    
    %% -----------------------------------------------------------------------
    %  SAVE
    %  -----------------------------------------------------------------------
    save(fullfile(PathSave, 'Workload_Li_TimeNormalised_G2.mat'), ...
        ...
        'valid_subj', 'Segments', 'Axes', 'N_bins', 'G2_Liszt');
    ...
    saveas(gcf, fullfile(PathSave, 'Fig_TimeNorm_Total_G2.png'));
    
    %% --- FIG 2-3 : BY AXIS ---
    for iSig = 1:2
        if iSig==1, MatN3 = Norm_Axis_Accel; else, MatN3 = Norm_Axis_Jerk; end
        figure('Name', ['Fig_TimeNorm_ByAxis_G2_' sig_names{iSig}], ...
        ...
        sgtitle([sig_names{iSig} ' By Axis normalisé vs RPE — Temps normalisé (%)'], ...
            'FontSize', 12, 'FontWeight', 'bold');
        saveas(gcf, fullfile(PathSave, ['Fig_TimeNorm_ByAxis_G2_' sig_names{iSig} '.png']));
    end
    ...
    % pour avoir les 24 participants G2.
    fprintf('\nChargement des 26 participants pour BySegMod...\n');
    
    Mat_Seg_Accel_26 = NaN(length(G2_Liszt), N_bins, N_seg);
    Mat_Seg_Jerk_26  = NaN(length(G2_Liszt), N_bins, N_seg);
    Mat_RPE_26       = NaN(length(G2_Liszt), N_bins);
    valid_26         = false(length(G2_Liszt), 1);
    
    for iG26 = 1:length(G2_Liszt)
        iP26   = G2_Liszt(iG26);
        ...
        if isempty(t_cyc26), continue; end
    ...
    fprintf('%d / 24 participants charges pour BySegMod_26\n', sum(valid_26));
    ...
    for iSig = 1:2
        ...
        figure('Name', ['Fig_TimeNorm_BySegment_G2_' sig_names{iSig} '_26'], ...
        ...
        sgtitle([sig_names{iSig} ' By Segment normalisé vs RPE — Temps normalisé (%) — n=26 G2'], ...
            'FontSize', 12, 'FontWeight', 'bold');
    ...
    for iSig = 1:2
        ...
        for iAx = 1:3
            fig_name = sprintf('Fig_TimeNorm_BySegAxis_G2_%s_%s', sig_label, Axes{iAx});
