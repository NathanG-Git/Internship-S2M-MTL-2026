%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%   Visualization - EMG vs RPE vs Temps normalise (%) - G2         %%%%
%%%%   G2 Long Duration (n=23) - memes traitements que G1              %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% CHANGEMENTS v2 :
...
%  -----------------------------------------------------------------------
Mat_EMG    = NaN(length(G2_Liszt), N_bins, N_ch, N_vars);
Mat_RPE    = NaN(length(G2_Liszt), N_bins);
valid_subj = false(length(G2_Liszt), 1);
Mat_MVC    = NaN(length(G2_Liszt), N_ch);   % MVC par participant x canal
...
for iG = 1:length(G2_Liszt)
    ...
    % pas de correction RPE specifique pour G2
...
save([PathSave '\EMG_Li_TimeNormalised_G2.mat'], ...
    'Mat_EMG', 'Mat_RPE', 'valid_subj', 'Channels', 'VarNames', ...
    'N_bins', 'G2_Liszt', 'Mat_MVC');
fprintf('Sauvegarde : EMG_Li_TimeNormalised_G2.mat\n');
...
for iV = 1:N_vars
    ...
    sgtitle([strrep(VarNames{iV}, '_', ' ') ' vs RPE - Temps normalise (%) - G2 n=' num2str(length(Vi))], ...
...
%% -----------------------------------------------------------------------
%  SECTION 9 - COMMANDES CONSOLE : identifier outliers SampleEntropy
%  Copier-coller dans la console MATLAB apres avoir lance le script
%  -----------------------------------------------------------------------
%
%  load('EMG_Li_TimeNormalised_G2.mat');
%  Vi    = find(valid_subj);
%  iV_SE = find(strcmp(VarNames, 'SampleEntropy'));
%  Chans = {'Biceps','Triceps','DeltAnt','DeltMed','SupTrap'};
%  thresh = 3;   % changer a 2 pour etre plus strict
%
%  for iC = 1:5
%      Mat_v  = squeeze(Mat_EMG(Vi, :, iC, iV_SE));
%      mu_all = nanmean(Mat_v(:));
%      sd_all = nanstd(Mat_v(:));
%      fprintf('\n=== %s - SampleEntropy ===\n', Chans{iC});
%      fprintf('  Groupe : mean=%.4f  SD=%.4f  seuil=+/-%.4f\n', mu_all, sd_all, thresh*sd_all);
%      for j = 1:length(Vi)
%          iG_j   = Vi(j);
%          iP_j   = G2_Liszt(iG_j);
%          vals_j = Mat_v(j, :);
%          if max(abs(vals_j - mu_all)) > thresh * sd_all
%              fprintf('  OUTLIER S1%02d (iG=%d) : max_dev=%.4f  vals=[%s]\n', ...
%                  iP_j, iG_j, max(abs(vals_j-mu_all)), num2str(vals_j,'%.3f '));
%          end
%      end
%  end
